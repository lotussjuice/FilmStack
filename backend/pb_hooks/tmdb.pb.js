...
routerAdd("GET", "/api/tmdb/discover", (e) => {
    ...
    const voteCountGte = query.get("vote_count_gte") || "";
...
routerAdd("GET", "/api/tmdb/trending", (e) => {
    const rawToken = $os.getenv("TMDB_API_KEY");
    const token = rawToken ? rawToken.trim() : "";
    if (!token) {
        return e.json(500, { error: "TMDB_API_KEY no configurada" });
    ...
    const query = e.request.url.query();
    const timeWindow = query.get("time_window") || "week";
    const page = query.get("page") || "1";

    try {
        const url = `https://api.themoviedb.org/3/trending/movie/${timeWindow}?language=es-MX&page=${page}&api_key=${token}`;
        const res = $http.send({
            url: url,
            method: "GET",
            headers: { "Accept": "application/json", "User-Agent": "FilmStack/1.0" },
